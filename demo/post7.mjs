/* the boring tek — social clip #7, "one tip for your business".
   renders out/post7-1080x1920.mp4, with the voice and the effects in the file.
   tooling, not the site: nothing here ships, nothing here edits index.html.

   post6 is the template and this is the first clip built on the whole stack at
   once rather than on a stack that grew under it: voice first, captions cut from
   its word timestamps, one pictogram scene keyed to the same words, the mascot
   on the same clock, and a sound layer whose every cue comes out of those plans.
   nothing in this file is a hand timed number except the scene's own beats, and
   those are typed against the printed word list rather than guessed.

   ---------- what is different from post6 ----------

   1. **it is a third of the length.** 22.20s against 10.22s. post6 counts out
      three things; this counts out one, and the whole clip is the argument for
      doing one thing rather than five. the shape of the script is the shape of
      the advice.

   2. **there is one scene, and it evolves instead of cutting.** post6 hands off
      between five scenes on the silences the reading leaves. this one holds a
      single scene for the whole clip and changes what is in it: five squares
      arrive, four dim away, the one that is left gets a check cut into it, and a
      second lights up at the end. there is no handoff to hide a change behind,
      so every change has to be a change the voice is making at that moment.

   3. **the beat is a word rather than a number.** post6 marks `one.` `two.`
      `three.` — three cards that are numerals. this marks the card that is the
      word `one` and nothing else, which the script lands once, at 4.87s, on
      `not five. one.` See The beat below for why it is once and not twice.

   4. **the dim is a new step kind's worth of behaviour.** `fade` could only go
      to nothing or to everything; four squares fading to 18% needed it to go
      somewhere in between, and needed it to stop overwriting the opacity of the
      step before it. Both are fixed in `lib/pictograms.mjs` and post6 is
      unaffected, because post6 has no `fade` steps at all.

   vertical only, for the reason post6 is: a caption, a head and a wordmark do
   not fit inside a 1080 tall frame with the air each of them needs.

     node post7.mjs                  the clip
     DEMO_FPS=12 node post7.mjs      the fast preview pass
     node post7.mjs --encode-only    re-encode from kept frames
*/
...
const FRAMES = path.join(OUT, 'frames-post7');
...
/* ---------- the script ----------
   one piece of advice, said once, with the counter argument built into it. the
   angle is the same as post6's and the shape is the opposite: post6 is a list of
   three, this is an argument against lists.

   `not five. one.` is the line the whole clip turns on and it is written as two
   sentences on purpose. the synthesiser reads a full stop as most of a second of
   air, so `one.` arrives alone, after a pause, on its own card, and the caption
   engine cuts it that way without being told to — a sentence that is one word
   long is a card that is one word long.

   no dashes, and `planCaptions` checks that rather than trusting this comment. */
const SCRIPT = 'one tip for your business. '
  + 'start with one boring task. not five. one. '
  + 'automate it. see it work. then take the next.';

/* ---------- the beat ----------
   the card that is the word `one` and nothing else. it is a word rather than a
   numeral, which is the one thing that makes this different from post6's beat
   test, and it is anchored at both ends so it cannot match `one` inside a
   longer card.

   **it lands once, at 4.87s.** the brief asked for both times it lands alone and
   the script only lands it alone once: `one` is also said at 0.12 and at 2.20,
   but both of those are inside a phrase — `one tip` and `one boring task` — and
   a card is cut at a sentence end or at two words, so neither ever gets a card
   of its own. that is not the emphasis rule being narrow, it is the copy: for a
   second beat the script would have to say `one.` alone a second time. the
   guard below finds the standalone `one` cards on its own, without the regexp,
   and fails if any of them is not a beat, so if the copy ever gains one this
   file will treat it as a beat without being edited. */
const BEAT = /^one$/i;
...
/* ---------- the cut ----------
   ...
   the vertical budget, top to bottom:
      175..361   the pictogram scene, 310 wide and centred
...
/* it is post6's final position, to the pixel, and it is written out here rather
   than imported because a clip owns its own frame — post2, post4 and post5 each
   do the same. if post6's block moves again this one does not follow it by
   accident, which is the behaviour worth having. */
...
/* the clip runs on past the last word, so it does not cut on a full stop. the
   mascot is still moving and still blinking through all of it. it is longer than
   post6's 0.65 because the last thing this clip does is put a second square on
   screen, and a payoff needs a second to be looked at. */
const TAIL = 1.10;
...
/* ---------- the eye choreography ----------
   the same performance post6 gives, over less than half the time: he is below
   the captions, so up is negative --ey, and he spends the clip reading them.

   there is one beat rather than three, so there is one lift rather than three,
   and it goes to the same -2.3 post6 saves for a beat. at 7.80, on `then take
   the next`, he leaves the captions and comes to the viewer, and stays. one
   look at the person watching in ten seconds, on the line that is addressed to
   them. deadpan, no widening: the line is the reaction.
   ...
   2.32 combined, which is post6's 2.33 to within a hundredth — the same calm,
   deliberately, because it is the same mascot listening to the same kind of
   advice. */
const EYE_KEYS = [
  [0.00, 0.0], [0.90, 0.0],       /* up at the words from frame zero */
  [1.50, -1.1], [2.30, -1.1],     /* a drift, reading across */
  [2.95, 0.9], [3.75, 0.9],
  [4.35, -0.8], [4.85, -0.8],
  [4.95, 0.3], [6.00, 0.3],       /* up to the beat, `one` at 4.99, and holds it */
  [6.60, 1.2], [7.30, 1.2],
  [7.80, 0.0], [9.60, 0.0],       /* comes to the viewer on `then`, and stays */
  [10.22, -0.5],                  /* still moving when the file ends */
];
const EYE_Y_KEYS = [
  [0.00, -1.6], [0.90, -1.6],
  [1.50, -1.9], [2.30, -1.9],
  [2.95, -1.5], [3.75, -1.5],
  [4.35, -1.8], [4.85, -1.8],
  [4.95, -2.3], [6.00, -2.3],     /* the highest he looks, and only on the beat */
  [6.60, -1.7], [7.30, -1.7],
  [7.80, -0.5], [9.60, -0.5],     /* level with the viewer */
  [10.22, -0.35],
];
...
/* ---------- the blinks ----------
   ...
   run. this is post7's own seed, post6's cadence and a much slower cadence than post5's: 3.0 to
...
/* ---------- the pictogram scene ----------
   one scene, held for the whole clip, changing as the voice changes.

   post6 cuts between five scenes on the silences the reading leaves, and every
   change it makes is hidden inside a handoff. this has no handoffs, so every
   change is in plain sight and every one of them has to be something the voice
   is doing at that moment. the times below are read off the printed word list:

     one tip for your business.   0.12 .. 0.75    squares one and two arrive
     start with one boring task.  1.75 .. 2.89    three, four and five arrive
     not five.                    3.90 .. 4.06    all five are on screen for it
     one.                         4.99            four dim, the middle one stays
     automate it.                 5.75 .. 6.13    the check is cut into it
     see it work.                 6.74 .. 6.98    it holds, which is the point
     then take the next.          7.79 .. 8.21    a second square lights up

   **the line names what is on screen when it is said.** `not five` lands with
   exactly five squares up, and `one` lands on the frame four of them start to
   go. that is the whole trick and it is why the times are typed against the
   word list rather than spread evenly.

   the composition is a centred band rather than a full block. five squares
   across a 100 unit board caps each of them at about 16 units with a gap worth
   having, so the scene is 16 units of a 60 unit box and the rest is air. that is
   what `five small squares in a row` is, and stretching it to fill the block
   would mean five large squares in a row, which is a different picture.

   the ink is rationed as it is everywhere else. one accent, on the square that
   lights up at the end, because that is the one thing the clip is asking for.
   the check is `cut` rather than accent: it is cut *into* the solid square the
   way the writing is cut into a document, and dark green on near black is the
   smudge post6 already learnt about. */
const HAIR = WEIGHTS.hair, MARK = WEIGHTS.mark;
/* how far down the four that lose go. 0.18 is low enough to read as gone and
   high enough that the row is still a row of five, which is the point: the
   other four did not stop existing, they stopped being the one being done. */
const DIM = 0.18;

export const SCENES = [
  {
    /* it holds to the last frame. `out` is the clip's own length and the guard
       below fails the render if the two ever disagree. */
    id: 'tasks', in: 0.10, out: 10.22,
    parts: [
      /* five, left to right, on the rhythm of the first two sentences. */
      {
        id: 'task-1', shape: 'square', at: { cx: 10, cy: 30, s: 16 },
        steps: [{ kind: 'pop', t: 0.35 }, { kind: 'fade', t: 5.02, for: 0.42, to: DIM }],
      },
      {
        id: 'task-2', shape: 'square', at: { cx: 30, cy: 30, s: 16 },
        steps: [{ kind: 'pop', t: 0.90 }, { kind: 'fade', t: 5.14, for: 0.42, to: DIM }],
      },
      /* the one that survives, and the only one with a single step. */
      { id: 'task-3', shape: 'square', at: { cx: 50, cy: 30, s: 16 }, steps: { kind: 'pop', t: 1.80 } },
      {
        id: 'task-4', shape: 'square', at: { cx: 70, cy: 30, s: 16 },
        steps: [{ kind: 'pop', t: 2.25 }, { kind: 'fade', t: 5.14, for: 0.42, to: DIM }],
      },
      {
        id: 'task-5', shape: 'square', at: { cx: 90, cy: 30, s: 16 },
        steps: [{ kind: 'pop', t: 2.95 }, { kind: 'fade', t: 5.02, for: 0.42, to: DIM }],
      },
      /* on `automate it`. cut into the square rather than laid on it, so it is a
         hole with the page showing through — the same trick as a document's
         writing, and the only one that reads on solid ink. */
      { id: 'done', shape: 'check', ink: 'cut', w: MARK, at: { cx: 50, cy: 30, s: 11 },
        steps: { kind: 'draw', t: 5.78, for: 0.55 } },
      /* on `then`. a second part sitting exactly where the fourth square is,
         rather than the fourth square changing colour, because a part has one
         ink for its whole life and this is the honest way to say that the next
         task is a different thing from the one that was dimmed. it is drawn
         after it in the dom, so it covers it. */
      { id: 'next', shape: 'square', ink: 'accent', at: { cx: 70, cy: 30, s: 16 },
        steps: { kind: 'pop', t: 7.82 } },
    ],
  },
];

/* the four that dim go outside in, 5.02 then 5.14, rather than all at once. a
   row that drops together is a light switch; a row that closes from the ends
   towards the middle is the field narrowing to the one that is left, which is
   what the line means. 0.12s apart is under a fifth of a second and reads as one
   move with a direction rather than as two moves.
...
pictogramPage();
window.__P7 = ${JSON.stringify({ VW, VH, WORDMARK_W })};
${scenePage.toString()}
scenePage();
</script>
</body>
...
function scenePage() {
  ...
  function fitWordmark() {
    ...
    el.style.fontSize = (window.__P7.WORDMARK_W / em).toFixed(3) + 'px';
  }
  ...
  window.__p7 = {
    ...
    safe() {
      const cap = window.__cap.safe(window.__P7.VW, window.__P7.VH);
      let out = { ...cap };
      /* the pictograms measure their own drawn ink, part by part, exactly as
         ...
         inside it. */
      const pic = window.__pic.safe(window.__P7.VW, window.__P7.VH);
      if (pic) {
      ...
      for (const sel of ['.mascot', '#wordmark']) {
        ...
        const d = {
          ...
          right: window.__P7.VW - b.right, bottom: window.__P7.VH - b.bottom,
        };
    ...
    zone() {
      ...
      const ceil = window.__p7.capCeil;
      return {
  ...
  document.fonts.load('400 1em Michroma')
    ...
    .then(() => {
      ...
      window.__p7.capCeil = capCeiling();
      window.__p7.ready = true;
    });
}
...
async function voice() {
  const cached = path.join(VOICE_OUT, 'post7-calm.json');
  ...
  const r = await speak(SCRIPT, { voice: 'calm', name: 'post7' });
...
async function render(plan, pic, seconds, blinks, picMotion) {
  ...
  console.log('  post7-1080x1920: ' + VW * DSF + 'x' + VH * DSF + ', ' + N + ' frames');
  ...
  let burned = 0;
  for (let i = 0; i < 150; i++) {
    const ok = await page.evaluate(() => !!(window.__p7 && window.__p7.ready
  ...
  if (!await page.evaluate(() => !!(window.__p7 && window.__p7.ready))) {
  ...
  const capCeil = await page.evaluate(() => window.__p7.capCeil);
  const boxes = await page.evaluate(() => window.__p7.boxes());
  console.log('  cards fitted at ' + built.size.toFixed(1) + 'px, the beat at '
  ...
  let picTicks = 0, picVisMax = 0, picMoved = 0, picStirred = 0, picApplied = 0, picPrevSum = null;
  const picFaults = [];
  let picPrev = null;
  ...
  for (let f = 0; f < N; f++) {
    ...
    const seen = await page.evaluate((fr, pf) => {
      ...
      window.__pic.set(pf);
      const accent = window.__p7.accent();
    ...
    if (picPrev) {
      ...
      for (let i = 0; i < picF.p.length; i++) {
        ...
        /* movement is not the only way a layer can be alive, and in this clip it
           is not one of them: nothing in this scene translates. squares spring,
           four of them dim, a check draws and one lights up, and every one of
           those is a channel other than dx and dy. the liveness guard runs on
           this rather than on `picMoved`, which would fail a scene that is
           working exactly as drawn. */
        for (let c = 0; c < b.length; c++) {
          const e = Math.abs(b[c] - a[c]);
          if (e > picStirred) picStirred = e;
        }
    ...
    const [eye, picLast] = await page.evaluate((ex, ey, bl) =>
      [window.__p7.life(ex, ey, bl), window.__pic.last],
    ...
    while (picNext < picSamples.length && t >= picSamples[picNext].t) {
      ...
      const z = await page.evaluate(() => window.__p7.zone());
      if (!z) continue;
    ...
    for (const s of samples) {
      ...
      sampled.add(s.i);
      const [sa, cl] = await page.evaluate(() => [window.__p7.safe(), window.__p7.clearance()]);
  ...
  console.log('  scenes: ' + picTicks + ' rAF ticks for ' + N + ' frames, at most '
    ...
    + '), biggest change on any channel ' + picStirred.toFixed(3) + ', '
  ...
  fs.writeFileSync(path.join(OUT, 'post7-1080x1920.json'), JSON.stringify(state, null, 2));
  return state;
}
...
function encode(audioFile) {
  const out = path.join(OUT, 'post7-1080x1920.mp4');
...
function sampleFrames(mp4, at) {
  const dir = path.join(OUT, 'verify-post7');
...
/* ---------- go ---------- */
async function main() {
  console.log('the boring tek — social clip #7, the honest advice post');
  ...
  /* `capSize` is 30 here where the engine's default is 40, and it is the beat
     that asks for it. this copy is short — the widest ordinary card is
     `automate it` — so the fit never runs out of box and every card lands on
     whatever the cap is. at the default that put the ordinary cards at 39.6px
     against a beat capped at the brand's 44, which is an eleven percent jump: a
     card very slightly larger than its neighbours, which reads as a wobble
     rather than as emphasis. at 30 the beat is 1.47x, which is post6's 1.55
     within a fraction, and the two clips look like siblings rather than like one
     clip and a louder one.

     the beat is what sets the caption size in a clip with short copy. that is
     worth knowing before the next script is written. */
  const plan = planCaptions(v.words, { style: 'pop', perCard: 2, capSize: 30, emphasise: BEAT });
  console.log(describe(plan));

  const beats = plan.groups.filter(g => g.big);
  /* found without the regexp, on purpose. the emphasis test is what *applies*
     the beat and this is what checks it was applied to everything it should
     have been, so the two have to disagree if the option is ever dropped. */
  const onesAlone = plan.groups.filter(g => g.words.length === 1 && /^one$/i.test(g.words[0].word));
  console.log('  beats: ' + (beats.length
    ? beats.map(g => '"' + g.words.map(w => w.word).join(' ') + '" at '
      + g.words[0].start.toFixed(2) + 's').join(', ')
    : 'none') + '  —  "one" lands on a card of its own ' + onesAlone.length + ' time(s)');
  ...
  /* the one number in the scene table that is also somewhere else. the scene
     holds to the last frame, so its `out` is the clip's length, and the clip's
     length is the voice plus a tail — change the script and this is the line
     that says so instead of the layer quietly ending early. */
  if (Math.abs(pic.seconds - SECONDS) > 0.005) {
    console.error('\n  FAILED — the scene ends at ' + pic.seconds.toFixed(2)
      + 's and the clip is ' + SECONDS.toFixed(2) + 's long. set the scene\'s `out` to match.');
    process.exit(1);
  }
  ...
  const cues = [
    ...
    /* `lastStep` rather than post6's `settle`: there is one scene here and it
       runs the whole clip, so the swell belongs on the last thing it does
       rather than under all of it. see cuesFromScenes for the argument. */
    ...cuesFromScenes(pic, { impact: IMPACT, seconds: SECONDS, humAt: 'lastStep' }),
  ].sort((a, b) => a.t - b.t);
  ...
  /* ---------- the loudness pass ----------
     ...
     one of them. */
  const wav = path.join(OUT, 'post7-mix.wav');
  const base = mix.out.slice();
  const passes = [];
  let lift = 0, after = null, lim = null;
  /* six rather than post6's four. limiting is not linear, so each pass gives
     back a little less than it asks for, and this clip's crest is high enough
     that four left it 0.7dB short. it still stops the moment it is within 0.3. */
  for (let i = 0; i < 6; i++) {
    mix.out.set(base);
  ...
  const state = ONLY_ENCODE
    ? JSON.parse(fs.readFileSync(path.join(OUT, 'post7-1080x1920.json'), 'utf8'))
  ...
  const dir = sampleFrames(file, [
    ...
    [4.20, 'c0-not-five'],
    [5.70, 'd0-one-stays'],
    [6.50, 'e0-the-check-is-cut'],
    [8.60, 'f0-the-next-lights-up'],
    [0.70, 'a-opening-card'],
    [2.60, 'b-one-boring-task'],
    [4.10, 'c-not-five'],
    [5.20, 'd-the-beat'],
    [6.00, 'e-automate-it'],
    [7.10, 'f-see-it-work'],
    [8.10, 'g-looks-at-you'],
    [Math.min(SECONDS - 2 / FPS, plan.seconds + 0.30), 'h-caption-clear'],
    [SECONDS - 2 / FPS, 'i-last-frame'],
  ]);
  ...
  console.log('  the beat lands at '
    + (beats.length ? beats.map(g => g.words[0].start.toFixed(2)).join(', ') + 's' : 'nowhere'));
  console.log('  the mascot comes to the viewer at 7.80 and stays');
  ...
  /* the beat, checked from the other end: find every card that is the word
     `one` and nothing else, without the regexp that marks them, and fail if any
     of them is not a beat or if the script stopped landing one at all. */
  if (!onesAlone.length) {
    fail.push('the script never lands "one" on a card of its own, so there is no beat to give');
  }
  if (onesAlone.some(g => !g.big)) {
    fail.push('a card that is just "one" did not get the beat treatment');
  }
  if (beats.length !== onesAlone.length) {
    fail.push('found ' + beats.length + ' beat cards against ' + onesAlone.length
      + ' cards that are just "one" — the emphasis test is matching something else');
  }
  ...
  if (!pg) fail.push('the render wrote no scene layer state at all');
    ...
    if (!(pg.stirred > 0.0001)) fail.push('no part of the scene layer ever changed on any channel');
     ...
     silent and would otherwise only show up as a clip that went quiet. */
  {
    ...
    const deep = sfx.report.filter(r => r.kind === 'popDeep').length;
    if (deep !== onesAlone.length) {
      fail.push(deep + ' deep pops for ' + onesAlone.length + ' beat card(s)');
    }
    /* only what this clip's scene can actually cue. post6 draws a coin, a
       shackle and a magnifier and checks for all three; this one draws squares
       and a check, so a `coin` here would mean a rule matched something it
       should not have. */
    if (!sfx.report.some(r => r.kind === 'ding')) fail.push('nothing in the scene cued a "ding"');
    for (const k of ['coin', 'click', 'sweep']) {
      if (sfx.report.some(r => r.kind === k)) {
        fail.push('a "' + k + '" was cued by a scene that has nothing to make one');
      }
    }
  }
